...
import time
import datetime
import warframe.translation as translation
import re
...
def get_info(message_from_user):
	...
	message_to_send=""
	if (message_from_user==[] or message_from_user==""):
		return ""
	...
	if (message_from_user[0]=="cx" or message_from_user[0]=="查询"):
		...
		if (message_from_user[1]=="alerts" or message_from_user[1]=="警报"):
		...
		elif (message_from_user[1]=="conclave" or message_from_user[1]=="PVP" or message_from_user[1]=="武形秘仪"):
			message_to_send=get_conclave_challenge()
		...
		elif (message_from_user[1]=="flashsales" or message_from_user[1]=="商店"):
			message_to_send=get_flash_sales()
			...
			message_to_send=get_invasions()+get_construction_progress()
		...
		elif (message_from_user[1]=="sentient" or message_from_user[1]=="S船"):
		...
		elif (message_from_user[1]=="arcane" or message_from_user[1]=="附能"):
			message_to_send=get_arcane_enhancement()
...
def get_arbitration():
	...
	if (data=={}):
		...
		if pick_out==[]:
			node=translation.trans_zh(node)
		else:
			pick_out=translation.trans_zh(pick_out)[0]
			pick_out="("+pick_out+")"
		...
		mission_type=translation.trans_zh(mission_type)
...
def get_cetus_mission():
	...
	if data==[]:
		...
		for i in range(job_number):
			...
			job_reward=translation.trans_zh(job_reward)
			reward_length=len(job_reward)
			
			# 赏金名
			job_type=jobs[i].get("type")
			job_type=translation.trans_zh(job_type)

			message_to_send=message_to_send+"\n赏金"+str(i+1)+"："+job_type+"，稀有奖励："+job_reward[reward_length-1]
		
		message_to_send="\n------希图斯赏金信息------"+message_to_send+"\n剩余时间："+rest_time

	return message_to_send

def get_conclave_challenge():
...
def get_darvo_daily_deal():
	...
	if data==[]:
		...
		data=data[0]
		item=data.get("item")
		item=translation.trans_zh(item)
		expiry=data.get("expiry")
...
def get_events():
	...
	if data==[]:
		...
		for i in range(all_events_length):
			...
			description=translation.trans_zh(description)
			...
			if (interimsteps_length!=0):
				for n in range(interimsteps_length):
					...
					if rewards_list==[""]:
						...
						rewards_message=""
						rewards_list=translation.trans_zh(rewards_list)
	...
	if data==[]:
		...
		for i in range(all_fissures_length):
			...
			if (data[i].get("tier")=="Lith"):
				...
				missiontype=translation.trans_zh(missiontype)
				# 敌人类型
		...
		for i in range(all_mission_length):
			if (data[i].get("syndicate")=="Steel Meridian"):
				...
				for n in range(nodes_length):
					...
					if pick_out==[]:
						single_node=translation.trans_zh(single_node)
					else:
						pick_out=translation.trans_zh(pick_out)[0]
						pick_out="("+pick_out+")"
						pattern=re.compile(r'[(](.*?)[)]')
